...
{
	std::ifstream config_FILE("D:\\DoProjects\\Search_Engine_proba\\JSON_files\\config.json");
...
{
	std::ifstream requests_FILE("D:\\DoProjects\\Search_Engine_proba\\JSON_files\\requests.json");
...
void ConverterJSON::PutAnswers(std::vector<std::vector<struct RelativeIndex>> answers)
{
	...
	std::ofstream answers_FILE("D:\\DoProjects\\Search_Engine_proba\\JSON_files\\answers.json", std::ios::trunc);
